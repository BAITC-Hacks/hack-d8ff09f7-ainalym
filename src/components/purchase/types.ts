...
export type Components = {
  ...
  on_hand?: Quantity;
  in_transit?: Quantity;
  stockout_uplift?: Quantity;
  stockout_months?: string[];
  outliers_excluded?: Outlier[];
  raw_need?: Quantity;
  moq?: Quantity;
  days_of_cover?: Quantity;
...
export type RecommendationsResponse = ResultAxes & {
  groups: RecommendationGroup[];
  run_id?: string;
  empty_reason?: string;
};
export type AgentAction = ResultAxes & {
  id: string;
  kind: string;
  summary_ru: string;
  rationale_ru?: string;
...
export type LedgerResponse = ResultAxes & {
  rows?: AgentAction[];
  actions?: AgentAction[];
  stats?: { auto: number; needs_you: number };
};
export type SeriesPoint = {
  ym: string;
  qty_file: Quantity;
  qty_regular: Quantity;
  stock: Quantity;
  stockout: boolean | number;
  outliers?: Outlier[];
};
...
export function money(value?: Money | string | null, currency = "KZT"): string {
  if (value === null || value === undefined || value === "")
    return "Себестоимость не задана";
...
export function axesOf(result?: ResultAxes): TruthAxes {
  const value = { ...result, ...result?.labels, ...result?.axes };
  return {
    provenance:
      value.provenance === "partner_anonymised" ||
      value.provenance === "synthetic"
        ? value.provenance
        : undefined,
    ai: ["live", "rules", "replay", "unavailable"].includes(value.ai ?? "")
      ? value.ai
      : undefined,
    external: ["export_only", "local_simulator", "unavailable"].includes(
      value.external ?? "",
    )
      ? value.external
      : undefined,
