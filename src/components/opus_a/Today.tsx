...
type TodayResp = TruthAxes & { lead: string; queue_count: number; pulse: { stockout_risk: { count: number; top: { code_1c: string; name: string; days_of_cover: number; lead_time_days: number; urgency?: string }[] }; agents: { auto: number; needs_you: number; ratio: number } }; empty_reason?: string; state_version: number };
...
function Metric({ label, value, unit, sub, href, loading, children }: { label: string; value?: string; unit?: string; sub?: React.ReactNode; href?: string; loading?: boolean; children?: React.ReactNode }) {
  return <div className="oa-metric">
    ...
    {loading ? <><Skel w="60%" h={34} /><Skel w="80%" h={14} /></> : <>
      <div className="oa-metric-value">{value}{unit ? <small>{unit}</small> : null}</div>
...
function DecisionCard({ item, onDone }: { item: QueueItem; onDone: (msg: { text: string; po?: string; kind: "ok" | "info" }) => void }) {
  ...
  async function openConfirm() {
    ...
    try {
      ...
      setPhase("ready");
    } catch (e) { setError(asErr(e)); setPhase("error"); }
  }
  const close = () => { setPhase("idle"); setError(null); opener.current?.focus(); };
  async function decide(key: "approve" | "reject") {
    ...
    try {
      ...
      onDone(key === "approve" ? { text: `${item.title} — черновик заказа создан. Черновик заказа — не отправлен.`, po, kind: "ok" } : { text: `${item.title} — предложение отклонено.`, kind: "info" });
      refresh();
    } catch (e) { setError(asErr(e)); setPhase("error"); }
  }
  ...
  return <li className="oa-card oa-decision" data-kind={item.kind} onKeyDown={e => { if (e.key === "Escape" && open) { e.stopPropagation(); close(); } }}>
    ...
    <div className="oa-decision-amount">
      ...
      {isProposal ? <span className="muted" style={{ font: "var(--oa-meta)" }}>сумма заказа</span> : null}
    </div>
    <div className="oa-decision-actions">
      {isProposal ? <>
        <button ref={opener} type="button" className="oa-btn oa-btn-black oa-btn-sm" aria-expanded={open} onClick={() => open ? close() : openConfirm()}>{open ? "Скрыть" : "Решить…"}</button>
        {supplier ? <Link className="oa-btn oa-btn-outline oa-btn-sm" href={`/opus_a/replenishment?supplier=${encodeURIComponent(supplier)}`}>Изменить количества</Link> : null}
    ...
    {open && isProposal ? <div className="oa-confirm" role="group" aria-label="Подтверждение решения">
      ...
      {detail && (phase === "ready" || phase === "busy" || phase === "error") ? <>
        ...
        <div className="oa-decision-actions" style={{ gridColumn: "auto" }}>
          <button autoFocus type="button" className="oa-btn oa-btn-black" disabled={phase === "busy"} onClick={() => decide("approve")}>{phase === "busy" ? "Сохраняю…" : "Подготовить заказ"}</button>
          <button type="button" className="oa-btn oa-btn-outline" disabled={phase === "busy"} onClick={() => decide("reject")}>Отклонить</button>
          <button type="button" className="oa-btn oa-btn-ghost" onClick={close}>Отмена <span className="oa-kbd" aria-hidden>Esc</span></button>
          <span className="muted" style={{ font: "var(--oa-meta)" }}>Решение привязано к версии v{detail.version}. Создаётся внутренний черновик — поставщику ничего не отправляется.</span>
        </div>
      </> : null}
  ...
  return <main className="oa-page" id="main">
    <div className="oa-head">
      ...
      <div className="oa-head-actions">
        <Link href="/opus_a/replenishment" className="oa-btn oa-btn-primary">Проверить заказы<ArrowRight size={16} aria-hidden /></Link>
      </div>
    </div>
      ...
      <aside className="oa-rightrail" aria-label="Деньги и агенты">
        ...
        <section className="oa-rail-block" aria-labelledby="oa-agents">
          ...
          {ledger.data?.rows.length ? <ol className="oa-feed" aria-label="Журнал агентов">{ledger.data.rows.map(r => <li key={r.id}>
            ...
            <div style={{ minWidth: 0 }}><div className="t">{r.summary_ru}</div><div className="m">{r.autonomy === "auto" ? "сам" : "ждёт вас"}{r.code_1c ? <> · <Link className="oa-link" href={`/opus_a/skus/${encodeURIComponent(r.code_1c)}`}>{r.code_1c}</Link></> : null}{r.provider ? ` · ${r.provider}` : ""}</div></div>
