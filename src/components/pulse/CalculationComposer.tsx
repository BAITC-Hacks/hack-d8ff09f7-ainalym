...
export function CalculationComposer() {
  const [supplier, setSupplier] = useState(""); const [category, setCategory] = useState(""); const [runId, setRunId] = useState<string>(); const action = useApiAction();
  return <section className={styles.composer} id="calculation" aria-labelledby="calculation-title"><div><h2 id="calculation-title">Рассчитать пополнение</h2><p className={styles.meta}>Продажи, остатки и товары в пути → заказ с обоснованием</p></div>
    <form className={styles.calcForm} onSubmit={async e => { e.preventDefault(); const result = await action.run(() => apiRequest<{ run_id: string; skus: number; recommended: number }>("/api/calc/run", { method: "POST", body: JSON.stringify({ scope: { ...(supplier ? { supplier } : {}), ...(category.trim() ? { category: category.trim() } : {}) } }) }), result => `Расчёт завершён · ${result.skus} товаров · ${result.recommended} рекомендаций`); if (result?.run_id) setRunId(result.run_id); }} aria-busy={action.busy}>
    ...
    </form><ActionStatus error={action.error} receipt={action.receipt} />{runId && <Link className={styles.inlineLink} href={`/purchases?run_id=${encodeURIComponent(runId)}`}>Открыть рекомендации расчёта</Link>}
  </section>;
}
