...
export function composeBody({ kind, code, value, text }: ComposeValues) {
  ...
  return { kind, code_1c, actor_id: "judge", text: text.trim(), payload: kind === "judge_message" ? { code_1c, qty: value, one_off: true } : kind === "in_transit_update" ? { code_1c, delta: value } : { code_1c, unit_cost: value } };
}
export function WorldControls() {
  ...
  const description = kind === "judge_message" ? `Разовый заказ ${value} шт по коду ${code}` : kind === "in_transit_update" ? `Товар в пути +${value} по коду ${code}` : `Себестоимость по коду ${code} = ${value} (в валюте товара)`;
  return <section className={styles.controls} aria-label="Воспроизвести или сочинить событие"><div className={styles.controlsHead}><div><h2>Следующее событие</h2><p className={styles.meta}>Событие меняет данные и запускает обработку агентом.</p></div><WorldLabel /></div>
