...
function LineDiff({ line, before, changed, editable, quantity, onEdit }: { line: OrderLine; before?: OrderLine; changed: boolean; editable: boolean; quantity?: string; onEdit: (value: string) => void }) {
  ...
  return <div className={`${review.diff} ${changed ? review.changed : ""}`}>
    ...
    <details className={styles.disclosure}><summary>Почему · источник</summary><p>{line.rationale_ru || "Обоснование не указано."}</p><Link className={styles.linkButton} href={`/skus/${encodeURIComponent(line.code_1c)}`}>История и расчёт SKU</Link></details>
  </div>;
}
