...
export function ResultCard({ title, response }: { title: string; response: AssistantResult }) {
  ...
  return <article className={styles.result} aria-label={title}>
    ...
    {data.run_id && <><p>Расчёт подготовлен{typeof data.recommended === "number" ? ` · рекомендаций: ${data.recommended}` : ""}.</p><p className={styles.meta}>Черновик заказа — не отправлен</p><Link className={styles.resultLink} href={`/replenishment?run_id=${encodeURIComponent(data.run_id)}`}>Открыть расчёт <ArrowUpRight size={14} aria-hidden="true" /></Link></>}
