...
export function ConnectionsPage() {
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.section} aria-labelledby="services-heading"><div className={styles.sectionHead}><h2 id="services-heading">Сервисы и каналы</h2></div>
      <div className={styles.serviceRow}><Cpu size={18} aria-hidden="true" /><div><h3>Провайдер решений{health.data?.ai_provider ? ` · ${health.data.ai_provider}` : ""}</h3><p className={styles.meta}>{ai ? label(`ai_${ai}`) ?? "Режим AI не передан" : "Режим AI не подтверждён"}</p></div><Link href="/assistant">ИИ-Помощник<ArrowUpRight size={14} aria-hidden="true" /></Link></div>
