...
function Shell({ children }: { children: React.ReactNode }) {
  const pathname = usePathname(); const router = useRouter(); const [collapsed, setCollapsed] = useState(false); const [view, setView] = useState<"commands" | "help" | "more" | null>(null);
  const { data: modes, error: modesError } = useApi<ModesResponse>("/api/modes"); const { offline, syncError, refresh } = useApiSync();
  const toggleRail = () => setCollapsed(value => { try { localStorage.setItem("ainalym:rail", String(!value)); } catch {} return !value; });
  useEffect(() => {
    const stored = localStorage.getItem("ainalym:rail"); if (stored === "true") setCollapsed(true);
    let chord = 0;
    const keydown = (e: KeyboardEvent) => {
      ...
      if (e.key === "[") toggleRail();
      if (Date.now() - chord < 1000) { const item = navigation.find(n => n.key === e.key); if (item) { router.push(item.href); chord = 0; return; } }
  ...
  const modeChip = <ModeChip mode={modes?.mode} ai={modes?.axes?.ai ?? modes?.ai} />;
  return <div className={styles.shell} data-collapsed={collapsed}>
    <a className={styles.skip} href="#main">К содержимому</a>
    <aside className={styles.rail} aria-label="Навигация приложения"><Link href="/today" className={styles.brand}>{collapsed ? "А" : "Айналым"}</Link>
      <nav className={styles.navigation} aria-label="Основные разделы">{navigation.map((item,i) => <Link href={item.href} key={item.href} className={`${styles.navItem} ${i === 5 ? styles.navSecondary : ""}`} aria-current={selected(item.href) ? "page" : undefined} title={collapsed ? item.label : undefined}><item.icon size={18} /><span>{item.label}</span></Link>)}</nav>
