...
type Sku = { code_1c: string; supplier_id: string; name: string; unit: string | null; moq: number; first_sale_ym: string | null; months_with_sales: number | null; median_month_qty: string | null; p95_doc_qty: string | null;
...
export async function computeNeed(code_1c: string, params: EngineParams, ctx: EngineContext = {}): Promise<NeedResult> {
  ...
  const sku = database.prepare("SELECT code_1c,supplier_id,name,unit,moq,first_sale_ym,months_with_sales,median_month_qty,p95_doc_qty,on_hand_qty,on_hand_as_of FROM sku WHERE code_1c=?").get(code_1c) as Sku | undefined;
  ...
  const firstSaleYm = sku.first_sale_ym ?? months.find((month) => dec(month.qty_file ?? month.qty_regular).gt(0))?.ym
    ?? sales.find((sale) => dec(sale.qty).gt(0))?.at.slice(0, 7) ?? null;
  const levelMonths = months.filter((month) => month.ym < monthOf(asOf) && (!firstSaleYm || month.ym >= firstSaleYm));
  ...
  const fileMonths = levelMonths.map((month) => dec(month.qty_file)).filter((qty) => qty.gt(0));
  ...
  for (const doc of docGroups.values()) {
    ...
    if (outlierCandidate && positiveDocs.length < 3) {
      ...
      if (doc.source === "file") excludedFromFile.set(doc.ym, (excludedFromFile.get(doc.ym) ?? new Decimal(0)).plus(doc.qty));
      continue;
    }
  ...
  const series = levelMonths.map((month) => ({
    ym: month.ym,
  ...
  if (!series.length) {
    for (const [ym, qty] of [...byMonth].sort(([a], [b]) => a.localeCompare(b)))
      if (ym < monthOf(asOf) && (!firstSaleYm || ym >= firstSaleYm)) series.push({ ym, qty, stockout: false, stockout_kind: null });
  }
  ...
  const components = {
    ...
    stockout_months: stockoutMonths, observed_stockout_months: observedStockoutMonths,
  ...
  const undatedTransit = transitRows.some((row) => !row.expected_at);
  const rationale_ru = `Код 1С ${code_1c}: фактические продажи ${components.raw_demand_rate} ${unit}/мес; спрос с учётом дефицита ${components.corrected_demand_rate} ${unit}/мес; сезонность ${ownSeason ? "артикула" : "поставщика"}, рост ×${components.growth}; прогноз на ${horizonDays} дн ${components.forecast_qty} + запас ${components.safety} − остаток ${components.on_hand} − в пути ${components.in_transit} − утверждённый заказ ${components.approved_order_qty} ${unit} = потребность ${need} ${unit} (${orderRule} ${sku.moq} ${unit}). ${arrivals.length ? `Ожидаемые поставки: ${arrivals.join(", ")}. ` : ""}${undatedTransit ? "Срок прибытия не указан — учтено в горизонте. " : ""}Месяцы с подтверждённым дефицитом: ${observedStockoutMonths.join(", ") || "нет"}; предполагаемый дефицит при неизвестном остатке: ${inferredStockoutMonths.join(", ") || "нет"}; исключены разовые документы: ${excluded.map((doc) => doc.doc_no).join(", ") || "нет"}.${pendingReview.length ? ` Документы ${pendingReview.map((doc) => doc.doc_no).join(", ")}: проверить вручную — недостаточно истории для исключения.` : ""}${stockStale ? ` Последний подтверждённый остаток ${stockMonth}; текущий остаток неизвестен, заказ требует уточнения.` : ""}`;
  return { forecast: { horizon_months: horizonDays / 30, base_rate: components.base_rate, season: components.season, growth: components.growth, stockout_uplift: components.stockout_uplift, safety: components.safety, method_ru: "Сезонный спрос × рост; восстановление спроса при дефиците; исключение разовых документов" }, need, rationale_ru, components, flags: components.flags };
}
