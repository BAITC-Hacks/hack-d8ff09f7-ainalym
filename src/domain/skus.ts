...
export async function skuView(code_1c: string): Promise<Record<string, unknown> | null> {
  ...
  for (const row of sales) months.set(String(row.ym), { ym: row.ym, qty_file: row.qty_file, qty_lines: row.qty_lines, qty_regular: row.qty_regular, stockout: row.stockout, outliers: [] });
  for (const row of stock) months.set(String(row.ym), { outliers: [], ...months.get(String(row.ym)), ym: row.ym, stock: row.known ? row.opening_qty : null, stock_known: Boolean(row.known) });
  for (const row of outliers) {
    const ym = String(row.at || "").slice(0, 7);
    const item = months.get(ym) || { ym, outliers: [] };
  ...
  return {
    sku,
    series: [...months.values()].sort((a, b) => String(a.ym).localeCompare(String(b.ym))),
    forecast: forecast ? { ...forecast, season: parsed(forecast.season) } : null,
    recommendation: recommendation ? { ...recommendation, components: parsed(recommendation.components) } : null,
    in_transit, timeline,
  };
}
