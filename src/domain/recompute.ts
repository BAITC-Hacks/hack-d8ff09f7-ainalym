...
export async function recomputeAffected(codes: string[], agentRunId?: string, worldEventId?: string): Promise<{ affected_codes: string[]; run_id: string | null; recommendation_ids: string[]; results: Record<string, { need: number; rationale_ru: string }>; proposal_ids: string[]; unresolved: { code_1c: string; supplier_id: string; reason: string }[] }> {
  const d = db();
  ...
  const run = await runCalculation({ codes: affected_codes }, {}, { agent_run_id: agentRunId, world_event_id: worldEventId });
