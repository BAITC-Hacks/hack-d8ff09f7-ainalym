...
export async function recomputeAffected(codes: string[], agentRunId?: string, worldEventId?: string): Promise<{ affected_codes: string[]; run_id: string | null; recommendation_ids: string[]; results: Record<string, { need: number; rationale_ru: string }>; proposal_ids: string[] }> {
  const d = db();
