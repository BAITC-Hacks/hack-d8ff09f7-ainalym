...
export default function SupplierActions({ poId, orderState, channel }: { poId: string; orderState: string; channel: { state: "draft" | "sent" | "confirmed"; reply_text: string | null } }) {
  ...
  if (channel.state === "confirmed") return <><p className={styles.body}>{channel.reply_text}</p><p className={styles.truth}>Reply received · Подтверждено (симулятор). Оригинальный ответ сохранён.</p></>;
