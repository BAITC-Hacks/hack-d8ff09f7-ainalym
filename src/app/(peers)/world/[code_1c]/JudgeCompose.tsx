...
function preset(kind: Kind, code: string, threshold: number) {
  if (kind === "judge_message") { const qty = Math.max(5000, Math.ceil(threshold * 1.1)); return { text: `Разовый заказ ${qty} шт по коду ${code}`, number: String(qty) }; }
...
export default function JudgeCompose({ code, threshold }: { code: string; threshold: number }) {
  ...
  const [text, setText] = useState(preset("judge_message", code, threshold).text);
  const [number, setNumber] = useState(preset("judge_message", code, threshold).number);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const [result, setResult] = useState<{ id: string; state: string; replayed: boolean } | null>(null);
  function choose(next: Kind) { const p = preset(next, code, threshold); setKind(next); setText(p.text); setNumber(p.number); setResult(null); }
  async function submit(event: React.FormEvent) {
    ...
    const payload = kind === "judge_message"
      ? { code_1c: code, qty: number, document_qty: qty, threshold, doc_type: "Расходная накладная", at: priorMonth }
