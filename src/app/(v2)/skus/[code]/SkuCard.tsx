  ...
  return <>
    ...
    <dl className={styles.strip} aria-label="Запас и прогноз">
      ...
      <Metric label="В пути" value={fmtNum(rec?.in_transit ?? transitTotal)} unit={unit}
        sub={in_transit.length ? `${in_transit.length} поставк${in_transit.length === 1 ? "а" : "и"} · дата поставки ${fmtDate(in_transit[0].expected_at)}` : "открытых поставок нет"} />
      ...
      <div className={styles.main}>
        ...
        <Section id="sources" title="Источники и даты" aside={<Truth>каждое число сверху выводится из этих строк</Truth>}>
          <Card className={styles.listCard}><Rows>
            ...
            <Row label={<Fact icon={<FileSpreadsheet size={15} aria-hidden />}>В пути (файл поставок)</Fact>} meta={in_transit[0]?.source_file ? basename(in_transit[0].source_file) : "открытых поставок нет"} value={fmtQty(transitTotal, unit)} valueMeta={in_transit.length ? `дата поставки ${fmtDate(in_transit[0].expected_at)}` : "—"} />
        ...
        {outliers.length > 0 && <Section id="outliers" title="Исключённые разовые документы" count={outliers.length} aside={<Truth>{comp.outlier_threshold ? `крупнее ${fmtNum(comp.outlier_threshold)} ${unit} считаем разовым заказом` : "необычно крупные документы считаем разовыми заказами"}</Truth>}>
