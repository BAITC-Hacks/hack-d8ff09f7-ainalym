  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.head}>
        ...
        <div className={styles.headActions}><Button variant="secondary" onClick={() => { t.reload(); q.reload(); }}><RefreshCw size={14} aria-hidden="true" />Обновить</Button><Link href="/v2/replenishment" className={styles.primaryLink}>Пополнение<ArrowRight size={15} aria-hidden="true" /></Link></div>
      </div>
        ...
        <aside className={styles.rail}>
          {d && <>
            <section className={styles.railBlock} aria-labelledby="risk-h">
              <div className={styles.sectionHead}><h2 id="risk-h" className={styles.h3}>Риск дефицита</h2><Link href="/v2/replenishment?urgency=critical" className={styles.more}>Все {fmtInt(d.pulse.stockout_risk.count)} <ArrowRight size={13} aria-hidden="true" /></Link></div>
...
function Pulse({ d, stale }: { d: Today; stale: boolean }) {
  ...
  return (
    <section className={styles.strip} aria-label="Пульс" data-stale={stale || undefined}>
      ...
      <Link href="/v2/replenishment?urgency=critical" className={`${styles.tile} ${styles.tileLink}`} title="SKU, чьё покрытие меньше срока поставки · /api/today">
    ...
    <article className={styles.card} aria-labelledby={`c-${item.id}`}>
      ...
      {!receipt && (
        <div className={styles.cardActions}>
          ...
          <Link href={`/v2/replenishment?supplier=${encodeURIComponent(item.title.match(/поставщику (\S+)/)?.[1] ?? "")}`} className={styles.reviewLink}>Смотреть позиции<ArrowRight size={14} aria-hidden="true" /></Link>
        </div>
      )}
    </article>
  );
