...
export function SkuCard({ code }: { code: string }) {
  ...
  useRowKeys(rail);
  if (loading && !data) return <><PageHead crumbs={[{ href: "/v2/replenishment", label: "Пополнение" }, { label: code }]} title={<span className={styles.ghost}>Загружаю…</span>} /><Loading label="Загружаю карточку позиции…" /></>;
  if (error && !data) return <><PageHead crumbs={[{ href: "/v2/replenishment", label: "Пополнение" }, { label: code }]} title={error.status === 404 ? "Позиция не найдена" : "Карточка недоступна"} />
...
function SkuBody({ data, reload, rail, stale }: { data: SkuResponse; reload: () => void; rail: React.RefObject<HTMLDivElement | null>; stale: boolean }) {
  ...
  return <>
    ...
    <PageHead crumbs={[{ href: "/v2/replenishment", label: "Пополнение" }, { label: sku.supplier_name ?? sku.supplier_id }, { label: sku.code_1c }]}
