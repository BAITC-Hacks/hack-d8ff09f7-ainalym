              ...
              {data.committed_by_supplier.map(c => {
                ...
                return <div key={c.supplier_id} className={styles.rowGroup}>
                  <div className={styles.tr} role="row" data-row tabIndex={0}>
                    <span role="cell" className={styles.cellMain}><span className={styles.name}>{SUPPLIER[c.supplier_id] ?? c.supplier_id}</span><span className={styles.meta}>{order ? <Link href={`/v2/supplier/${order.id}`}>{order.id.slice(0, 11)}… · ETA {fmtDate(order.eta)}</Link> : `заказ ${c.supplier_id}`}</span></span>
