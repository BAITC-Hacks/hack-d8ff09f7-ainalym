...
export async function GET(_request: Request, context: { params: Promise<{ code: string }> }): Promise<Response> {
  return handle(async () => {
    ...
    if (view) return ok({ ...view, sku: { ...sku, ...(view.sku as Record<string, unknown>), image_url: skuImageUrl(sku) ?? product?.image_url ?? null }, ekt });
    ...
    return ok({ sku: { ...sku, image_url: skuImageUrl(sku) ?? product?.image_url ?? null }, series, forecast, recommendation, in_transit, timeline, ekt });
  });
}
