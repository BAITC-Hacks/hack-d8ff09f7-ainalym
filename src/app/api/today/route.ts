...
export async function GET(): Promise<Response> {
  return handle(async () => {
    ...
    const commitments = Array.isArray(today.commitments) && today.commitments.length ? today.commitments : lastRun ? [{ id: lastRun.id, kind: "run", title: `Расчёт: ${lastRun.recommended} рекомендаций`, next_event: "Решение менеджера", owner: "purchasing_manager", state: "needs_review" }] : [];
    ...
    return ok({ ...today, lead: today.lead || (lastRun ? `Рекомендаций к заказу: ${lastRun.recommended}` : ""),
      ...
      ...(lastRun ? { empty_reason: undefined } : {}) });
  });
}
