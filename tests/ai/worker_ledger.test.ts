...
beforeAll(() => {
  ...
  d.prepare(`INSERT INTO world_event(id,org_id,seq,kind,po_id,source_id,at,text,payload,state)
    VALUES ('WE-REPLY','OWN',2,'supplier_reply','PO-TEST','REPLY-1',?,'Предоплата 50%',?,'pending')`)
    .run(today, JSON.stringify({ po_id: "PO-TEST" }));
  d.prepare(`INSERT INTO world_event(id,org_id,seq,kind,source_id,at,payload,state)
    VALUES ('WE-BAD','OWN',3,'unsupported','BAD-1',?,'{}','pending')`).run(today);
});
afterAll(() => {
  resetInstance();
...
it("writes one run per event, decision metadata, and a failure reason with L1 ledger", async () => {
  const result = await tick();
  expect(result.processed).toBe(2);
  const runs = db().prepare("SELECT trigger_ref,state,actions_count,escalations_count FROM agent_run WHERE trigger_type='world_event' ORDER BY started_at,id").all() as
  ...
  expect((db().prepare("SELECT COUNT(*) AS n FROM agent_run WHERE trigger_type='world_event'").get() as { n: number }).n).toBe(3);
});
