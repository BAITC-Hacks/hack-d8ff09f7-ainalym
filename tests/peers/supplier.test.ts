...
describe("controlled supplier channel", () => {
  it("holds a draft, records a local send, and preserves the reply verbatim", async () => {
    expect(supplierChannel("PO-1").channel).toMatchObject({ state: "draft", label: "Черновик заказа — не отправлен", external: "local_simulator" });
    const sent = await supplierReply("PO-1", { action: "send_demo" });
    expect(sent.channel.state).toBe("sent");
    expect(sent.channel.label).toBe("Отправлено (контролируемый демо-канал)");
    ...
    expect(first.channel.label).toBe("Подтверждено (симулятор)");
