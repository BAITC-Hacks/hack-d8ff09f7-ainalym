...
const money: MoneyView = { cash: [], committed_by_supplier: [{ supplier_id: "IEK", amount: "0.00", currency: "KZT", lines: 4, cost_known_lines: 0 }], next_60d: { out: [{ at: "2026-10-02", amount: "30.00", currency: "CNY", po_id: "PO-two", kind: "supplier_balance" }, { at: "2026-10-01", amount: "10.50", currency: "KZT", po_id: "PO-one", kind: "supplier_prepayment" }] }, stock_value: null, risks: [{ code: "cost_unknown", count: 4, label_ru: "Себестоимость не задана" }] };
...
describe("a_money page", () => {
  ...
  it("sorts persisted payments by date and keeps currency totals separate", async () => {
    ...
    render(<MoneyPage initial={money} />); expect(screen.getByText("Остатки не заданы")).toBeTruthy(); const links = screen.getAllByRole("link").filter(link => link.getAttribute("href")?.startsWith("/purchases/")); expect(links.map(link => link.textContent)).toEqual(["PO-one", "PO-two"]); expect(screen.getAllByText("30 CNY")).toHaveLength(2); expect(screen.getAllByText("10,50 ₸")).toHaveLength(2); await screen.findByText("Правила без LLM");
  });
});
