...
| W2 | Ответ поставщика → предложение разделить/ускорить заказ, 30/70 по частям | `src/ai/supplier-reply.ts` · событие `WE-046` · `/review/:id` | `npm run check` → `supplier_reply …`; `node scripts/scenario.mjs` → `[PASS] World (applied=46/46)` | GREEN · 11:10Z |
