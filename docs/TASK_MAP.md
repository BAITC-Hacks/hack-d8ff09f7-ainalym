...
Наблюдение: **2026-09-23 10:39Z**, `main` @ `3b3dcbd` (после MERGE-2, L5 voice, L4a/L4b/L4c, L6, L3, L8, INTEG-1). Чистый клон: `/tmp`, `.env.local` из `.env.example` + `DATABASE_PATH=./data/partner.db`, без ключей.
...
| M5 | Список по поставщикам + обоснование + экспорт для 1С | `GET /api/recommendations` · `/replenishment` · `GET /api/orders/:id/export.xlsx` (форма «Заказ поставщику», «Номенклатура.Код») | `node scripts/scenario.mjs` → `[PASS] M5-full`, `[PASS] M5 (rows=566)`; API: экспорт 294 строки, срочность 58/83/153 | GREEN · 10:39Z |
...
| C2 | Сводная проверка | `scripts/check.mjs` | `npm run check` → `check: passed=250 failed=0 skipped=7 externally-unverified=7` | GREEN · 10:39Z (чистый клон `3b3dcbd`) |
| C3 | Чистый клон | `scripts/clean_clone_check.sh` (клон → install → etl → check) | локальный клон `3b3dcbd` → `CLEAN-CLONE: PASS` (passed=250 failed=0 skipped=7 externally-unverified=7) | GREEN · 10:39Z (GitHub-remote — ≈12:10Z) |
